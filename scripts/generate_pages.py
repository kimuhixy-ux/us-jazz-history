...
BASE = "https://kimuhixy.com/us-jazz-history"
...
def schema(row: dict, genre: str, artist_type: str, english: bool) -> str:
    ...
    graph = [
        {"@type": "WebSite", "@id": f"{BASE}/#website", "url": f"{BASE}/", "name": "US Jazz History", "inLanguage": ["ja", "en"]},
...
def detail_context(row: dict, genre: str, related: list[int], rows: list[dict], artist_types: dict[str, str], english: bool) -> dict[str, str]:
    ...
    facts = "".join([
        ...
        f'<div><dt>{"Personnel" if english else "参加ミュージシャン"}</dt><dd>{esc(data["personnel"])}</dd></div>',
    ])
    ...
    meta = f"{title} by {artist}, released in {data['year']}. View genre and personnel facts in the US Jazz History essential album guide." if english else f"{artist}『{title}』（{data['year']}年）のジャンル、参加ミュージシャンをUS Jazz History名盤ガイドの事実情報で確認できます。"
    return {
        "lang": "en" if english else "ja", "title": esc(title), "artist": esc(artist), "genre": esc(genre),
        "page_title": esc(f"{title} — {artist} ({data['year']}) | US Jazz History"), "meta_description": esc(meta[:155]),
...
def index_context(rows: list[dict], labels: dict, english: bool) -> dict[str, str]:
    ...
    title = "Essential Jazz Album Index" if english else "ジャズ名盤索引"
    canonical = f"{BASE}/{prefix}items/"
    graph = [{"@type": "WebSite", "@id": f"{BASE}/#website", "url": f"{BASE}/", "name": "US Jazz History", "inLanguage": ["ja", "en"]}, {"@type": "CollectionPage", "url": canonical, "name": title, "inLanguage": "en" if english else "ja", "isPartOf": {"@id": f"{BASE}/#website"}}]
    return {
        "lang": "en" if english else "ja", "title": title, "page_title": f"{title} | US Jazz History",
        "meta_description": f"Browse {len(rows)} essential jazz albums by genre, with artist, year, and personnel facts." if english else f"ジャズの精選名盤{len(rows)}枚をジャンル別に探せる索引です。アーティスト、発表年、参加ミュージシャンを確認できます。",
