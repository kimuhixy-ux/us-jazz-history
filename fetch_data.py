...
# -*- coding: utf-8 -*-
"""
us-jazz-history: MusicBrainz APIからアメリカのジャズ系アーティスト情報を収集するスクリプト。

使い方:
  # 小規模テスト(シードリストの先頭10組のみ)
  python3 fetch_data.py --mode test

  # 全件取得(シード全70組程度 + タグ検索でアメリカ出身ジャズ系アーティストを300組以上収集)
...
USER_AGENT = "us-jazz-history/1.0 ( us-jazz-history-app@example.com )"
...
SEED_ARTISTS = [
    # トラディショナル・ジャズ / スウィング
    "Louis Armstrong", "King Oliver", "Jelly Roll Morton", "Sidney Bechet", "Bix Beiderbecke",
    "Duke Ellington", "Count Basie", "Benny Goodman", "Artie Shaw", "Glenn Miller",
    "Woody Herman", "Chick Webb", "Cab Calloway", "Lionel Hampton", "Coleman Hawkins",
    # ヴォーカル・ジャズ
    "Ella Fitzgerald", "Billie Holiday", "Sarah Vaughan", "Nat King Cole", "Nina Simone",
    "Mel Tormé", "Anita O'Day", "Frank Sinatra",
    # ビバップ
    "Charlie Parker", "Dizzy Gillespie", "Thelonious Monk", "Bud Powell", "Max Roach",
    "Kenny Clarke", "Fats Navarro",
    # ラテン・ジャズ
    "Machito", "Tito Puente", "Cal Tjader", "Mongo Santamaria",
    # クールジャズ
    "Miles Davis", "Chet Baker", "Dave Brubeck", "Gerry Mulligan", "Lennie Tristano",
    "Stan Getz", "Lee Konitz",
    # ハードバップ / ソウルジャズ
    "Art Blakey", "Horace Silver", "Sonny Rollins", "Clifford Brown", "Cannonball Adderley",
    "Lee Morgan", "Freddie Hubbard", "Wynton Kelly", "Jimmy Smith", "Les McCann",
    "Ramsey Lewis", "Stanley Turrentine", "Grant Green",
    # モード / フリー / ポストバップ
    "John Coltrane", "McCoy Tyner", "Bill Evans", "Ornette Coleman", "Cecil Taylor",
    "Albert Ayler", "Sun Ra", "Don Cherry", "Archie Shepp", "Wayne Shorter",
    "Herbie Hancock", "Andrew Hill", "Woody Shaw", "Joe Henderson", "Eric Dolphy",
    "Charles Mingus",
    # フュージョン
    "Weather Report", "Chick Corea", "John McLaughlin", "Return to Forever",
    "Mahavishnu Orchestra", "Pat Metheny", "Jaco Pastorius",
    # スムースジャズ / アシッドジャズ / コンテンポラリー
    "Grover Washington Jr.", "George Benson", "Kenny G", "Bob James", "David Sanborn",
    "US3", "Medeski Martin & Wood", "Robert Glasper", "Kamasi Washington",
    "Christian Scott", "Esperanza Spalding", "Wynton Marsalis", "Branford Marsalis",
    "Diana Krall",
]

# --- 広く収集するためのタグ検索キーワード(主要ジャンルを網羅) ---
TAG_QUERIES = [
    "jazz", "dixieland", "traditional jazz", "swing", "big band",
    "vocal jazz", "bebop", "latin jazz", "cool jazz", "hard bop",
    "soul jazz", "modal jazz", "free jazz", "avant-garde jazz", "post-bop",
    "fusion", "jazz fusion", "smooth jazz", "acid jazz", "nu jazz",
]
...
def get_studio_albums(mbid):
    ...
    注意: LPアルバムという形式自体が一般化したのは1940年代末以降のため、
    それ以前の初期ジャズ・アーティストは、この条件では「スタジオアルバム」が
    ほとんど(あるいは全く)ヒットしないことがある。これはデータの欠損ではなく、
    そもそも当時「アルバム」という単位で作品が発表されていなかったことによる。
    """
...
def build_artist_record(mbid):
    ...
    life_begin_year = int(begin[:4]) if begin and len(begin) >= 4 else None
    ...
    album_years = [a["year"] for a in albums if a["year"] is not None]

    # 個人アーティスト(person)は、MusicBrainzのlife-span begin(=生年)ではなく
    # 「最初のスタジオアルバムのリリース年」を活動開始の目安として使う。
    # ジャズは個人名義のリーダー作が中心のため、生年基準だと年表が
    # 実際の活躍時期と大きくずれてしまうことへの対策(バンド=groupは
    # 従来通りフォーメーション年であるlife-span beginを使う)。
    #
    # 注意: MusicBrainzでは、没後に編纂されたリイシュー/コンピレーション盤が
    # 誤って(secondary-typeなしの)"Album"として登録されているケースがある。
    # 存命中に出ていない「アルバム」を活動開始年として採用すると、
    # 例えば1938年に没したキング・オリヴァーが1998年発売の編集盤のせいで
    # 「1990年代のアーティスト」として扱われてしまうような明白な誤りが生じる。
    # そのため、没年(存命なら現在)以前にリリースされたアルバムのみを対象にする。
    lifetime_cutoff = end_year if end_year is not None else 2100
    valid_album_years = [y for y in album_years if y <= lifetime_cutoff]
    if artist_type == "person" and valid_album_years:
        begin_year = min(valid_album_years)
    else:
        begin_year = life_begin_year
...
def main():
    parser = argparse.ArgumentParser(description="us-jazz-history データ収集スクリプト")
